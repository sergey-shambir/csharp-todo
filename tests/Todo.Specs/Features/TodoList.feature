Функциональность: TODO-списки
    Как пользователь
    Я хочу вести свои списки дел
    Чтобы систематизировать ежедневную рутину

@positive
Сценарий: создание, редактирование, просмотр и удаление одного списка
    Пусть я создал список "Домашние дела"
    И добавил задачу "Убрать комнату"
    И добавил задачу "Съесть борщ"
    И добавил задачу "Приготовить борщ"
    И добавил задачу "Покормить хомяка"
    Когда я открыл созданный список
    Тогда вижу задачи: "Убрать комнату, Съесть борщ, Приготовить борщ, Покормить хомяка"

    Пусть я переместил задачу №3 на позицию №2
    Когда я открыл созданный список
    Тогда вижу задачи: "Убрать комнату, Съесть борщ, Покормить хомяка, Приготовить борщ"

    Пусть я переименовал задачу №1 на "Съесть борщ со сметаной"
    И переместил задачу №1 на позицию №3
    Когда я открыл созданный список
    Тогда вижу задачи: "Убрать комнату, Покормить хомяка, Приготовить борщ, Съесть борщ со сметаной"
    И вижу завершённые задачи: ""

    Пусть я завершил задачу №0
    И завершил задачу №2
    Когда я открыл созданный список
    Тогда вижу задачи: "Убрать комнату, Покормить хомяка, Приготовить борщ, Съесть борщ со сметаной"
    И вижу завершённые задачи: "Убрать комнату, Приготовить борщ"

    Пусть я удалил задачу №1
    Когда я открыл созданный список
    Тогда вижу задачи: "Убрать комнату, Приготовить борщ, Съесть борщ со сметаной"

@positive
Сценарий: поиск по строке в двух списках
    Пусть я создал список "Домашние дела"
    И добавил задачу "Убрать комнату"
    И добавил задачу "Съесть борщ"
    И добавил задачу "Приготовить борщ"
    И добавил задачу "Сварить кашу с молоком"

    Пусть я создал список "Покупки"
    И добавил задачу "Молоко"
    И добавил задачу "Булочка с кунжутом"
    И переименовал задачу №0 на "Свежее молоко"

    Когда я открыл списки задач
    Тогда вижу списки: "Домашние дела, Покупки"

    Когда я открыл списки задач с фильтром "борщ"
    Тогда вижу списки: "Домашние дела"

    Когда я открыл списки задач с фильтром "молоко"
    Тогда вижу списки: "Домашние дела, Покупки"

    Когда я открыл списки задач с фильтром "автомобиль"
    Тогда вижу списки: ""

@positive
Сценарий: просмотр и удаление одного из двух списков
    Пусть я создал список "Домашние дела"
    И добавил задачу "Убрать комнату"
    И добавил задачу "Приготовить борщ"
    И добавил задачу "Съесть борщ"

    Пусть я создал список "Покупки"
    И добавил задачу "Молоко"
    И добавил задачу "Булочка с кунжутом"

    Когда я открыл списки задач
    Тогда вижу списки: "Домашние дела, Покупки"

    Когда я перешёл к списку "Домашние дела"
    Тогда вижу задачи: "Убрать комнату, Приготовить борщ, Съесть борщ"

    Когда я перешёл к списку "Покупки"
    Тогда вижу задачи: "Молоко, Булочка с кунжутом"

    Пусть я удалил список "Домашние дела"

    Когда я открыл списки задач
    Тогда вижу списки: "Покупки"

    Когда я перешёл к списку "Покупки"
    Тогда вижу задачи: "Молоко, Булочка с кунжутом"
